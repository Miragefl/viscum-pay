...
public class Md5Util {
    ...
    public static String genDigest(String digestStr) throws NoSuchAlgorithmException {
        MessageDigest md;
        String pwd = "";
        // 生成一个sha1加密计算摘要 同样可以使用SHA1
        md = MessageDigest.getInstance("sha1");
        // 计算sha1函数
        md.update(digestStr.getBytes());
        // digest()最后确定返回sha1 hash值，返回值为8为字符串。因为sha1 hash值是16位的hex值，实际上就是8位的字符  BigInteger函数则将8位的字符串转换成16位hex值，用字符串来表示；得到字符串形式的hash值  pwd = new BigInteger(1, md.digest()).toString(16);参数也可不只用16可改动,当然结果也不一样了
